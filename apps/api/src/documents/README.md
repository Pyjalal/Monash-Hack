...
The wrapper unit tests use a deterministic subprocess fixture; the [#43 live verification suite](../../../../tools/eval/ocr-verification/README.md) in `npm test` requires Python and Tesseract. Run the Python regressions separately with `python -m unittest discover -s tools/ocr-sidecar -p 'test_*.py'`. Downstream OCR-aware field selection, comparison and vision escalation remain separate integration tickets; the Gmail native-source validator is unchanged.
