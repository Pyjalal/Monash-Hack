...
The wrapper unit tests use a deterministic subprocess fixture; the [#43 live verification suite](../../../../tools/eval/ocr-verification/README.md) in `npm test` requires Python and Tesseract. Run the Python regressions separately with `python -m unittest discover -s tools/ocr-sidecar -p 'test_*.py'`. The [operational OCR integration](../../../../docs/ocr-operational-integration.md) now connects a conservative labelled-field comparison path to Gmail and fresh pre-send validation. Unsupported layouts and unverified vision candidates remain blocked rather than authorizing a match.
