...
export const DOCUMENT_CASES = ['cedar', 'harbour'].flatMap((group, index) => {
  ...
  return [
    ...
    { name: 'wrong-document', sources: [si, bl.replace('DRAFT BILL OF LADING', 'COMMERCIAL INVOICE')], status: 'NEEDS_REVIEW', reason: 'wrong_doc_type', defects: [], workflow: 'BLOCKED', blockers: ['WRONG_DOC_TYPE', 'DOCUMENT_ROLES_UNVERIFIED'] },
