...
export function evaluationRoute(classification: RoutingClassification | null | undefined): EvaluationRoute {
  ...
  const confidence = typeof classification.expectationConfidence === "number" && Number.isFinite(classification.expectationConfidence)
    ? classification.expectationConfidence : 0;
  if (classification.expectation === "FUTURE_DRAFT" && confidence >= 0.8) return {
  ...
  if (classification.expectation === "VERIFY_NOW" && confidence >= 0.8) return {
  ...
  return {
    ...
    documentExpectation: "UNCERTAIN", workflowState: "BLOCKED", reason: "The timing expectation is missing or below the 80% operational threshold.",
  };
}
