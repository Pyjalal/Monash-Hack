...
export async function publishFinal(report: string, dataset: string, ledger: string, output: string, python: string) {
  ...
  // Component checks must exercise the frozen product implementation.
  for (const [path, digest] of Object.entries(manifest.config.implementations)) {
    if ((path.startsWith('apps/') || path.startsWith('packages/') || path === 'package-lock.json' || path === 'tools/eval/src/export.ts') && sha(await readFile(path)) !== digest) throw new Error(`Product changed since final run: ${path}`);
  }
