...
4. The answer model returns bounded structured claims and citation numbers. Code validates the shape and binds citations to server-selected sources.
...
Chat history stays in React memory and clears on reload or reset. Relevant email excerpts and questions go to the configured Jev and answer providers. Authentication, input limits (1000-character question, eight history messages, 40 KB body), a process-wide request budget and four concurrent answers bound access and cost. A network failure gets one generation retry within the same 18-second timeout. Rate limits should be shared at the edge for multi-process deployments.
